...
db.serialize(() => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS categories (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT,created_at DATETIME default CURRENT_TIMESTAMP,
        updated_at DATETIME default CURRENT_TIMESTAMP);`)
    db.run(`CREATE TABLE IF NOT EXISTS orders (id INTEGER PRIMARY KEY AUTOINCREMENT, custumer_id INTEGER, menu_id INTEGER, qty INTEGER, order_date DATE, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, update_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (customer_id) REFERENCES custumer(id), FOREIGN KEY (menu_id) REFERENCES menu(id));`)
})
